 ...
 *  x, y, z (world position), heading (radians, 0..1),
 *  fuelAmount, fuelCapacity, fuelConsumption, fuelRange, fuelWarning,
 *  gameTime (minutes since start), restStop (minutes until rest)
 */
...
struct telemetry_state_t
{
	...
	unsigned int game_time;      // in-game minutes since start
	bool  game_time_available;
	int   rest_stop;             // minutes until next required rest
	bool  rest_stop_available;
...
static void udp_send_json()
{
	...
	const int n = snprintf(
		...
		sizeof(buf),
						...
						telemetry.position_available ? telemetry.pos_x : 0.0f,
						telemetry.position_available ? telemetry.pos_y : 0.0f,
						telemetry.position_available ? telemetry.pos_z : 0.0f,
						telemetry.orientation_available ? telemetry.heading : 0.0f,
						telemetry.fuel_amount,
						telemetry.fuel_capacity,
						telemetry.fuel_consumption,
						telemetry.fuel_range,
						telemetry.fuel_warning ? "true" : "false",
						telemetry.nav_distance,
						telemetry.nav_time,
...
{
	const struct scs_telemetry_frame_start_t *const info = static_cast<const struct scs_telemetry_frame_start_t *>(event_info);
...
SCSAPI_VOID telemetry_frame_end(const scs_event_t UNUSED(event), const void *const UNUSED(event_info), const scs_context_t UNUSED(context))
{
	...
	#if defined(__linux__) || defined(_WIN32)
	static scs_timestamp_t last_udp_time = 0;
	const scs_timestamp_t now = telemetry.raw_rendering_timestamp;
	// Throttle UDP to ~10 Hz (100ms in game time which is microseconds)
	if (now - last_udp_time >= 100000) {
		last_udp_time = now;
		udp_send_json();
	}
	#endif
...
SCSAPI_VOID telemetry_store_placement(const scs_string_t UNUSED(name), const scs_u32_t UNUSED(index), const scs_value_t *const value, const scs_context_t context)
{
	assert(context);
	telemetry_state_t *const state = static_cast<telemetry_state_t *>(context);
	...
	state->orientation_available = true;
	state->pos_x = static_cast<float>(value->value_dplacement.position.x);
	state->pos_y = static_cast<float>(value->value_dplacement.position.y);
	state->pos_z = static_cast<float>(value->value_dplacement.position.z);
	state->heading = value->value_dplacement.orientation.heading;
	state->pitch   = value->value_dplacement.orientation.pitch * 360.0f;
	state->roll    = value->value_dplacement.orientation.roll * 360.0f;
}
...
SCSAPI_VOID telemetry_store_s32(const scs_string_t UNUSED(name), const scs_u32_t UNUSED(index), const scs_value_t *const value, const scs_context_t context)
{
	...
	*static_cast<int *>(context) = value->value_s32.value;
	...
	#if defined(__linux__) || defined(_WIN32)
	if (context == &telemetry.gear || context == &telemetry.displayed_gear || context == &telemetry.rest_stop) {
		udp_send_json();
	}
	#endif
}

SCSAPI_VOID telemetry_store_u32(const scs_string_t UNUSED(name), const scs_u32_t UNUSED(index), const scs_value_t *const value, const scs_context_t context)
{
	assert(value);
...
SCSAPI_VOID telemetry_store_u32_time(const scs_string_t UNUSED(name), const scs_u32_t UNUSED(index), const scs_value_t *const value, const scs_context_t UNUSED(context))
{
	assert(value);
	assert(value->type == SCS_VALUE_TYPE_u32);
	telemetry.game_time = value->value_u32.value;
	telemetry.game_time_available = true;
	#if defined(__linux__) || defined(_WIN32)
	udp_send_json();
	#endif
}

SCSAPI_VOID telemetry_store_s32_rest(const scs_string_t UNUSED(name), const scs_u32_t UNUSED(index), const scs_value_t *const value, const scs_context_t UNUSED(context))
{
	assert(value);
	assert(value->type == SCS_VALUE_TYPE_s32);
	telemetry.rest_stop = value->value_s32.value;
	telemetry.rest_stop_available = true;
	#if defined(__linux__) || defined(_WIN32)
	udp_send_json();
	#endif
}
...
SCSAPI_RESULT scs_telemetry_init(const scs_u32_t version, const scs_telemetry_init_params_t *const params)
{
	...
	version_params->register_for_channel(SCS_TELEMETRY_TRUCK_CHANNEL_fuel,                    SCS_U32_NIL, SCS_VALUE_TYPE_float, SCS_TELEMETRY_CHANNEL_FLAG_none, telemetry_store_float, &telemetry.fuel_amount);
	version_params->register_for_channel(SCS_TELEMETRY_TRUCK_CHANNEL_fuel_average_consumption, SCS_U32_NIL, SCS_VALUE_TYPE_float, SCS_TELEMETRY_CHANNEL_FLAG_none, telemetry_store_float, &telemetry.fuel_consumption);
	...
	version_params->register_for_channel(SCS_TELEMETRY_TRUCK_CHANNEL_fuel_warning,             SCS_U32_NIL, SCS_VALUE_TYPE_bool,  SCS_TELEMETRY_CHANNEL_FLAG_none, telemetry_store_bool,  &telemetry.fuel_warning);

	version_params->register_for_channel(SCS_TELEMETRY_TRUCK_CHANNEL_navigation_distance,      SCS_U32_NIL, SCS_VALUE_TYPE_float, SCS_TELEMETRY_CHANNEL_FLAG_none, telemetry_store_float, &telemetry.nav_distance);
	version_params->register_for_channel(SCS_TELEMETRY_TRUCK_CHANNEL_navigation_time,          SCS_U32_NIL, SCS_VALUE_TYPE_u32,   SCS_TELEMETRY_CHANNEL_FLAG_none, telemetry_store_u32,   &telemetry.nav_time);
	version_params->register_for_channel(SCS_TELEMETRY_TRUCK_CHANNEL_navigation_speed_limit,   SCS_U32_NIL, SCS_VALUE_TYPE_float, SCS_TELEMETRY_CHANNEL_FLAG_none, telemetry_store_float, &telemetry.nav_speed_limit);

	version_params->register_for_channel(SCS_TELEMETRY_CHANNEL_game_time,      SCS_U32_NIL, SCS_VALUE_TYPE_u32, SCS_TELEMETRY_CHANNEL_FLAG_none, telemetry_store_u32_time, NULL);
	version_params->register_for_channel(SCS_TELEMETRY_CHANNEL_next_rest_stop, SCS_U32_NIL, SCS_VALUE_TYPE_s32, SCS_TELEMETRY_CHANNEL_FLAG_none, telemetry_store_s32_rest, NULL);

	game_log = version_params->common.log;
	game_log(SCS_LOG_TYPE_message, "Initializing ETS2 telemetry UDP JSON plugin with game time/rest stop");
